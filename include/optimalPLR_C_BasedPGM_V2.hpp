...
protected:
     ...
     */
    static void build(const std::vector<K> &data,
        ...
        if (data.back() == sentinel)
            throw std::invalid_argument("The value " + std::to_string(sentinel) + " is reserved as a sentinel.");

        auto build_level = [&](auto epsilon, std::vector<K> input, std::vector<Segment> &output, size_t last_n) {
            auto n_segments = PGM_C2::internal::make_segmentation_par<K,int>(last_n, epsilon,input,output);
        ...
        while (epsilon_recursive && last_n > 1) {
            ...
            auto input = PGM_C2::internal::translate<K>(segments,offset,last);
     ...
     */
    PGMIndex(const std::vector<K> &data)
        : n(data.size()),
            segments(),
            levels_offsets(),
            first_key(data.empty() ? 0 : data.front()) {
