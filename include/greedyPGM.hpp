...
/**
 * A struct that stores the result of a query to a  PGMIndex, that is, a range [ lo, hi)
 * centered around an approximate position of the sought key.
 */
struct ApproxPos {
    ...
    std::vector<long> residuals; ///< The residuals at each recursive level.
};

/**
 * @tparam K the type of the indexed keys
 * @tparam Parallelism the number of threads to use for parallelization
...
protected:
    ...
    size_t Epsilon;             // The last-mile error bound for the segmentation
    size_t EpsilonRecursive;    // The internal error bound for the recursive segmentation
    struct Segment;
    ...
    /**
     * @brief A method to build the PGM index on the given range of keys.
     * @tparam RandomIt the type of the iterator over the keys
     * 
     */
...
struct PGMIndex<K, Parallelism, Floating>::Segment {
    ...
    explicit Segment(const typename internal::GreedyPiecewiseLinearModel<K, int>::CanonicalSegment &cs)
    : key(cs.first_x) {
