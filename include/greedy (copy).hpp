...
public:
    ...
    bool add_point(const X &x, const Y &y)
    {
        ...
        if (points == 1)
        {
            ...
            points++;
            double so_x = (end[0].x+initial[0].x)/2.0;
            double so_y = (end[0].y+initial[0].y+end[1].y+initial[1].y)/4.0;
        ...
        double slope1 = (y -epsilon - so.second) / (static_cast<double>(x) - so.first);
        double slope2 = (y +epsilon - so.second) / (static_cast<double>(x) - so.first);
