#include "greedy.hpp" // Update the path if greedy.hpp is located elsewhere
#include <iostream>
#include <vector>
#include <deque>
#include <limits>
 ...
 * @param data 
 */
void checkData(std::vector<K> data){
    for(int i = 0;i<=10;i++){
 ...
 */
Result experiment_GreedyPLR(std::vector<K> data,double epsilon = 32,int threads_num = 16){
    ...
    using Segment= Greedy::internal::GreedyPiecewiseLinearModel<K,int>::CanonicalSegment::Segment;
    ...
    size_t num_segments = Greedy::internal::make_segmentation_par(data.size(), epsilon, in, out_parallel,threads_num);
    ...
    // Print out the result of the parallel version
    printf("The number of segments in the parallel version is %d\n",result.seg_parallel);
    printf("The time taken for the parallel version is %f seconds\n",result.time_parallel);
    return result; 
 ...
 * 
 */
Result experiment_linear(std::vector<K> data,double epsilon = 32,int threads_num = 16){
    // Intialize the results
    Result result;
 ...
 */
Result experiment_OptimalPLR(std::vector<K> data,double epsilon = 32,int threads_num = 16){
    // Intialize the results
    Result result;
 ...
 */
Result experiment_SwingFilter(std::vector<K> data,double epsilon = 32,int threads_num = 16){
    ...
    // Initialize the GreedyPiecewiseLinearModel and OptimalPiecewiseLinearModel and OptimalPLR  
    using Segment= PGM_S::internal::SwingPiecewiseLinearModel<K,int>::CanonicalSegment::Segment;
    auto result_segments_serial = std::vector<Segment>();
    auto result_segments_parallel = std::vector<Segment>();
    ...
    // The time taken for the serial version and the number of the segments
    auto start = std::chrono::high_resolution_clock::now();
    size_t num_segments = PGM_S::internal::make_segmentation(data.size(), epsilon, in, out_serial);
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> serial_duration = end - start;
    result.time_serial = serial_duration.count();
    result.seg_serial = num_segments;
    ...
    start = std::chrono::high_resolution_clock::now();
    num_segments = PGM_S::internal::make_segmentation_par(data.size(), epsilon, in, out_parallel,threads_num);
    end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> parallel_duration = end - start;
    // The time taken for the parallel version and the number of the segments
    result.time_parallel = parallel_duration.count();
...
/**
 * @brief experiment with the ParaoptimalPLR 
 * 
 * @param data 
 * @param epsilon 
 * @param threads_num 
 * @return Result of the ParaoptimalPLR
 */
Result experiment_Paraoptimal(std::vector<K> data,double epsilon = 32,int threads_num = 16){
    ...
    // Initialize the GreedyPiecewiseLinearModel and OptimalPiecewiseLinearModel and OptimalPLR  
    using Segment= PGM_P::internal::ParaoptimalPiecewiseLinearModel<K,int>::CanonicalSegment::Segment;
    auto result_segments_serial = std::vector<Segment>();
    auto result_segments_parallel = std::vector<Segment>();
    ...
    // Print out the result of the serial version
    printf("The number of segments in the serial version is %d\n",result.seg_serial);
    printf("The time taken for the serial version is %f seconds\n",result.time_serial);

    // Check the segments
    PGM_P::internal::checkForEpsilon(in,result_segments_serial,4,9,epsilon);
 ...
 * @param threads_num 
 */
void experiment(std::vector<K> data,int start, int end,int threads_num = 16)
{
...
int main(){
    ...
    experiment_Paraoptimal(data, 4.0, 16);
    return 0;
}
